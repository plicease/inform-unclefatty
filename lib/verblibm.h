...
#IFV5; style roman; #ENDIF;
   ...
   print " library ", (string) LibRelease, "uf ";
#ifdef STRICT_MODE;
   print "s";
#endif;
#ifdef DEBUG;
...
[ VersionSub blarph;	!! GNR lower case it.
  Banner();
  blarph = 0->$1f;
  if(64 < blarph && blarph < 91) blarph = blarph + 32;
  !print "blarph = ", blarph, "^";
...
[ AttemptToTakeObject item     ancestor after_recipient i j k;
  ...
  k=0; objectloop (j in player)
	if (j hasnt worn) {
		if(j provides weight)
			k=k+j.weight;
		else
			k++;
	}
...
[ InsertSub ancestor;
  ...
  if (children(second) >= ValueOrRun(second,capacity))
	...
	!print "here... noun = ", (object) noun, "^second = ", (object) second, "^^";
